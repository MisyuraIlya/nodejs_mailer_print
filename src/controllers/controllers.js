...
const { generateConfig } = require("../enums/utils");
...
async function getAttachmetns(req, res){
  try {
    const url = `https://gmail.googleapis.com/gmail/v1/users/${req.params.email}/messages/${req.params.messageId}/attachments/${req.params.attachId}`;
    const { token } = await oAuth2Client.getAccessToken();
    const config = generateConfig(url, token);
    const response = await axios(config);
    res.type('application/pdf');
    res.header('Content-Disposition', `attachment; filename="invoice.pdf"`);
    res.send(Buffer.from(response.data.data, 'base64'));
  } catch (error) {
    console.log(error);
    return error;
  }
